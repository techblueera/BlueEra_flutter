...
**Entity types (`type` enum / `entityType` in results):**
`product`, `variant`, `grocery_product`, `grocery_shop`, `user`, `business`, `service`.
...
Prefer `deepLink` for navigation; fall back to `entityType` + `sourceId`:
