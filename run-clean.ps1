...
#
# Usage:
#   ./run-clean.ps1            # flutter run, spam stripped from the console
#   ./run-clean.ps1 --release  # any extra args are forwarded to `flutter run`
...
# Stream `flutter run` output and drop every line matching the noise filter.
& flutter run @args 2>&1 | Where-Object { $_ -notmatch $filter }
