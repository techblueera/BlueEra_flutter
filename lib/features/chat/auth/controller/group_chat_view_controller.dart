...
import '../socket/group_chat_socket.dart';

class GroupChatViewController extends GetxController {
  ...
  final chatSocket = GroupChatSocketService();
...
print("data1");
    ...
    chatSocket.listenEvent('newMessageReceived', (data) {
      ...
      // Ensure myMessage field is properly set
      if (message.myMessage == null) {
        // Compare sender ID with current logged-in user ID to determine if it's the user's message
        final currentUserId = userId; // Global variable from shared_preference_utils.dart
        final senderId = message.senderId;
        message.myMessage = currentUserId == senderId;
        print('GroupChatViewController: Setting myMessage - currentUserId: "$currentUserId", senderId: "$senderId", myMessage: ${message.myMessage}');
      }

        emitEvent("ChatList", {ApiKeys.type: "group"}, true);
  ...
  Future<bool?> sendOfflineMessageToServer(
    ...
    try {
      ...
      await ChatViewRepo().sendGroupMessageToUser(params);
  ...
  void emitEvent(String event, dynamic data,
    ...
    if (event == "ChatList" && isFromInitial != true) {
      final connectivityResult = await NetworkUtils.isConnected();
      // If OFFLINE, load from local cache and return; if ONLINE, fall through to emit
      if (!connectivityResult) {
  ...
  Future<bool> forwardMessageApi(
    ...
    ResponseModel responseModel =
    await ChatViewRepo().groupForwardMessageApi(params);
    clearMessageControllerCommon();
    if (responseModel.isSuccess) {
      clearMessageControllerCommon();
      return true;
  ...
  Future<bool> createGroupApi(
    ...
    if (responseModel.isSuccess) {
      // refresh group chat list after creating a group
      emitEvent("ChatList", {ApiKeys.type: "group"}, true);
      return true;
    } else {
  ...
  Future<bool> updateMessageApi(
    ...
    try {
      ResponseModel responseModel =
      await ChatViewRepo().updateGroupSingleMessage(params);
      clearMessageControllerCommon();
      if (responseModel.isSuccess) {
        clearMessageControllerCommon();
        return true;
  ...
  Future<void> deleteChatMessage(
    ...
    try {
      ResponseModel responseModel =
      await ChatViewRepo().deleteGroupSingleMessage(params);
      clearMessageControllerCommon();
      if (responseModel.isSuccess) {
      } else {
        clearMessageControllerCommon();
  ...
  Future<bool> likeAndUnlikeMessage(
    ...
    try {
      ResponseModel responseModel =
      await ChatViewRepo().groupLikeUnlikeSingleMessage(params);
      clearMessageControllerCommon();
  ...
  Future<void> generateUploadUrlsApi({
    ...
    try {
      ...
      ResponseModel responseModel =
      await ChatViewRepo().generateGroupUploadUrlsApi(params);
      clearMessageControllerCommon();
  ...
  Future<bool?> sendMessageLargeFile(Map<String, dynamic> params,
    ...
    try {
      ...
      ResponseModel responseModel =
      await ChatViewRepo().sendGroupMessageToUserLargeFile(params);
      clearMessageControllerCommon();
      if (responseModel.isSuccess) {
        if (sendFiles != null &&
            sendFiles.isNotEmpty &&
