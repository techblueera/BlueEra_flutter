...
class _SelfPickupMsgCardState extends State<SelfPickupMsgCard> {
  ...
  OrderCardContext get _cardContext => OrderCardContext(
        ...
        items: _boardItems,
        totalItemCount: widget.message.metadata?.selfPickupOrder?.totalItems,
  ...
  String? get _boardOrderNumber {
    final id = widget.message.metadata?.selfPickupOrder?.orderId ??
        widget.message.metadata?.selfpickupOrderId ??
        '';
  ...
  List<OrderCardItem> get _boardItems {
    final items = widget.message.metadata?.selfPickupOrder?.items ?? [];
    return [
  ...
  ///
  /// For grocery this is the only surface that shows the real, server-driven
  /// order state: `/track` works for this vertical and nothing else does
  ...
  void _onLifecycleChanged(OrderActionsModel? fresh) {
    ...
    if (status != null) {
      ...
      if (status == OrderStatusValue.ready) {
  ...
  /// The board card — `Your order has been placed successfully!` over
  /// `Order #0D1247`.
  Widget _boardCard() {
    return Row(
      mainAxisSize: MainAxisSize.min,
    ...
    return Row(
      ...
      child: Column(
        ...
        children: [
          ...
          // Server-driven lifecycle: banner, countdowns, payment sub-state,
          // refund wording and the action bar. Renders `availableActions` and
          // nothing else. Falls through to the card's original status row on a
          // legacy order that carries no `metadata.lifecycle`.
