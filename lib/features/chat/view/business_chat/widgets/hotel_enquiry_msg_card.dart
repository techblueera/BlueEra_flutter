...
/// In-chat card for `message_type: "hotel_enquiry"` — restyled to match the
/// service-enquiry reference: slim header, divider-separated sections
/// (photo / selections / note), and an accept-decline or status footer.
/// Data extraction, controller wiring, and actions are unchanged; only the
/// visual shell mirrors [ServiceEnquiryMsgCard].
...
class _HotelEnquiryMsgCardState extends State<HotelEnquiryMsgCard> {
  ...
  /// Non-empty selection groups (Room Type / Purpose / Amenities / Timeline
  /// or whatever the payload carries), each paired with a distinct tinted
  /// icon to mirror the reference enquiry-row treatment.
  List<_EnquiryRow> get _rows {
    final raw = _e?.selections ?? const <String, List<String>>{};
    final out = <_EnquiryRow>[];
    raw.forEach((title, values) {
      final items =
          values.where((s) => s.trim().isNotEmpty).toList();
  ...
  /// Icon + tint per known section title. Unknown labels fall back to the
  /// generic category tint so the row still renders on-brand.
  (IconData, Color) _iconAndColorFor(String title) {
    switch (title.toLowerCase()) {
      case 'room type':
      case 'room':
      case 'rooms':
        return (Icons.king_bed_rounded, const Color(0xFF3B82F6));
      case 'guests':
      case 'occupants':
        return (Icons.group_rounded, const Color(0xFF22C55E));
      case 'timeline':
      case 'stay dates':
      case 'dates':
      case 'check-in':
      ...
      case 'check in':
      case 'check out':
        return (Icons.calendar_month_rounded, const Color(0xFFF59E0B));
      case 'purpose':
      case 'type':
        return (Icons.travel_explore_rounded, const Color(0xFF8B5CF6));
      case 'amenities':
        return (Icons.star_rounded, const Color(0xFF0EA5E9));
      default:
  ...
  List<String> get _photos {
    final e = _e;
    final list = <String>[];
    final cover = (e?.listingImage ?? '').trim();
    if (cover.isNotEmpty) list.add(cover);
    for (final p in e?.photos ?? const <String>[]) {
      if (p.trim().isNotEmpty && !list.contains(p)) list.add(p);
    }
    return list;
  }

  String get _note => (_e?.note ?? '').trim();
  ...
  Widget _footer() {
    if (_isAccepted) {
      // Enquiry-first flow: once the owner accepts, the customer gets a
      // "Book Now" CTA that opens the booking sheet with `enquiry_id`
      // pre-filled so the resulting booking links back to this enquiry.
      return Column(
