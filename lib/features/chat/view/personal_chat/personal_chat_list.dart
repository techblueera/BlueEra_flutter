...
;    return Obx(() {
      if (chatViewController.personalChatListResponse.value.status == Status.COMPLETE) {
        GetChatListModel? data = chatViewController.getPersonalChatListModel?.value;
            ...
            child: (data?.chatList?.isEmpty??true)?noChatsFound():ListView.builder(
              itemCount: data?.chatList?.length,
                ...
                itemBuilder: (context, index) {
                  ...
                  },type:data?.chatList?[index]?.sender?.accountType ?? AppConstants.individual
                  ,index: index, chatViewController: chatViewController, chat: data?.chatList?[index], theme: theme, isForwardUI: widget.isForwardUI, context: context);
                },
            ),
          ),
        );
