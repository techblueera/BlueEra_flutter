...
extension VideoDataHiveMapper on VideoDataHive {
  VideoData toVideoData() {
    return VideoData(
      id: id,
      userId: userId,
      channelId: channelId,
      ...
      taggedUsers: taggedUser?.toTaggedUserList(),
...
extension AuthorHiveMapper on AuthorHive {
  Author toAuthor() {
    return Author(
      id: id,
...
extension ChannelHiveMapper on ChannelHive {
  Channel toChannel() {
    return Channel(
      websites: websites,
      id: id,
...
extension CategoryListMapper on List<CategoryHive>? {
  List<Categories> toCategories() =>
      this?.map((c) => c.toCategory()).toList() ?? [];
}

extension TaggedUserHiveMapper on TaggedUserHive {
  TaggedUser toTaggedUser() {
    return TaggedUser(
      id: id,
      username: username,
      accountType: accountType,
...
extension TaggedUserListHiveMapper on List<TaggedUserHive>? {
  List<TaggedUser> toTaggedUserList() =>
      this?.map((c) => c.toTaggedUser()).toList() ?? [];
}

extension LocationHiveMapper on LocationHive {
  Location toLocation() {
    return Location(
      name: name,
      lat: lat,
...
extension SongHiveMapper on SongHive {
  Song toSong() {
    return Song(
      id: id,
      name: name,
...
extension StatsHiveMapper on StatsHive {
  Stats toStats() {
    return Stats(
      views: views,
      likes: likes,
