...
class _OttVideoPlayerScreenState extends State<OttVideoPlayerScreen> {
  ...
  void initState() {
    ...
    if (widget.videoItems.videoUrl?.isNotEmpty ?? false) {
      controller.initializePlayer(widget.videoItems.transcodedUrls?.master??"");
    }
  ...
  Widget _buildQualitySettings(BuildContext context) {
    return Theme(
      data: Theme.of(context).copyWith(
        popupMenuTheme: const PopupMenuThemeData(color: Colors.black87),
      ),
      child: PopupMenuButton<String>(
        icon: const Icon(Icons.settings, color: Colors.white),
        onSelected: (quality) => _changeQuality(
          _qualities.firstWhere((track) => track.resolution == quality),
        ),
        itemBuilder: (context) => _qualities
            .map(
              (track) => PopupMenuItem<String>(
                value: track.resolution,
                child: Row(
                  children: [
    ...
    return Theme(
      ...
      child: PopupMenuButton<String>(
        ...
        itemBuilder: (context) => controller.videoQualityUrls.keys
            .map((quality) => PopupMenuItem(
                  ...
                  child: Row(
                    children: [
                      ...
                      const SizedBox(width: 8),
                      CustomText(quality, color: Colors.white),
                    ],
                  ),
                ))
            .toList(),
...
class _OttVideoPlayerScreenState extends State<OttVideoPlayerScreen> {
 ...
 // Inject Controller
  ...
  List<HlsTrack> _qualities = [];
  HlsTrack? _currentTrack; // null means "Auto"
  bool _isLoading = true;

  @override
  void initState() {
    super.initState();
    controller.initializePlayer("https://cph-p2p-msl.akamaized.net/hls/live/2000341/test/master.m3u8"); // Start with Auto
    _loadQualities();
  }

  Future<void> _loadQualities() async {
    final tracks = await parseHlsMasterPlaylist(widget.masterUrl);
    setState(() {
      _qualities = tracks;
    });
  }
              ...
              child: controller.isInitialized.value
                  ? Stack(
                      ...
                      children: [
                        ...
                        Obx(() => AnimatedOpacity(
                              opacity:
                                  controller.showControls.value ? 1.0 : 0.0,
                              duration: const Duration(milliseconds: 300),
                              child: IgnorePointer(
                                ignoring: !controller.showControls.value,
                                child: Stack(
