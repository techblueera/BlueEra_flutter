...
/// ## What this replaced
///
/// This page shipped as `DiscoverScreenV2`, beside an older Discover that was
/// a catalogue: alternating chip ROWS and FOLDER tiles, fourteen sections deep,
/// every one of them a way to browse. This one leads with what the user
/// already has around them — ten launchers, then the shops and people actually
/// nearby, then their own recent orders — and puts the full catalogue behind
/// those ten tiles.
///
/// The comparison is over: the old page is DELETED and this one took both its
/// name and its file. Every destination below was already the same call the old
/// page made — the same sheets, the same screens, the same category ids — so
/// nothing about where a tap lands changed when it went.
///
/// `RouteConstant.discoverV2Screen` still exists and now mounts this page, the
/// same as the bottom-nav tab; the `V2` in that route name is historical.
...
class _PinnedSearchBar extends SliverPersistentHeaderDelegate {
  ...
  static const double _pad = _DiscoverScreenState._sectionGap;
