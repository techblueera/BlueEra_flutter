...
class StoresNearYouAllScreen extends StatefulWidget {
  const StoresNearYouAllScreen({super.key, this.section, this.title});

  /// Which rail sent the user here, or null for the whole neighbourhood.
  ///
  /// Discover shows "Shops Near Me" and "Services Near Me" as two rails over
  /// two different lists, and both used to open this screen with no argument —
  /// so both landed on the merged set and each showed the other's businesses.
  /// Passing the section makes the screen the View All of the rail that opened
  /// it. Null keeps the old behaviour for the one entry point that really does
  /// mean "everything near you" — v1's Near You rail, which is a single merged
  /// rail rather than a pair.
  final NearbySection? section;

  /// Header, so the screen keeps the name the user just tapped. The rail titles
  /// come from the response (`shopsTitle` / `servicesTitle`), so they are passed
  /// in rather than restated here. Falls back to the generic "Near you".
  final String? title;

  @override
  State<StoresNearYouAllScreen> createState() => _StoresNearYouAllScreenState();
}
...
/// Corner radius of one card in the ladder — a row, its skeleton, and the ad
/// slot interleaved between them, which has to be the same object as the rows
/// around it rather than a panel with its own geometry.
const double _kCardRadius = 18;

/// Gap under every card in the ladder. One number, so the space above an ad
/// (the previous row's gap) and the space below it are the same by
...
class _StoresNearYouAllScreenState extends State<StoresNearYouAllScreen> {
  ...
  /// Everything this screen is allowed to show, before the chip filter — the
  /// one rail's list when opened from a rail, the merged set otherwise.
  List<NearbyEntry> _all() {
    final section = widget.section;
    return section == null
        ? buildNearbyEntries(_controller)
        : buildNearbySectionEntries(_controller, section);
  }
          ...
          (context, index) {
            ...
            if (row.isAd) {
              ...
              return NativeAdSlot(
                ...
                // Same gap the rows leave under themselves, so the ad sits in
                // the list's rhythm: the space above it (the previous row's
                // own gap) and the space below it are one number, not two
                // that happen to be close.
                margin: const EdgeInsets.only(
                    left: _NearbyRow._spineWidth, bottom: _kRowGap),
                // The rows are 18; matching it keeps the ad reading as one
                // more card in the ladder rather than a panel dropped into it.
