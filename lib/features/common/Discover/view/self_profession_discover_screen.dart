...
class _SelfProfessionDiscoverScreenState extends State<SelfProfessionDiscoverScreen> {
  ...
  Widget _buildServicesMap(double statusBarHeight) {
    // Frame the map on the location the user picked on the entry screen (if
    // any) so the map + pill agree with the radius-scoped results, rather than
    // always showing the device fix.
  ...
  Widget build(BuildContext context) {
    ...
    return AnnotatedRegion<SystemUiOverlayStyle>(
      // Dark status-bar icons — they sit over the light map header (which now
      // stays fixed at the top), matching the reference design.
      ...
      child: Scaffold(
        ...
        // The map header stays pinned at the top and the results list scrolls
        // only in the space below it. The category was chosen on the entry
        // screen (old pinned category strip removed), and a fixed header means
        // list content (e.g. the "… near you" title) can never scroll up under
        // the status bar.
        body: Column(
          children: [
            _buildServicesMap(statusBarHeight),
            Expanded(
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          // Title: "{Category} near you" — the category chosen on the entry
          // screen. Reactive so it updates if the selection changes.
          Obx(() {
  ...
  Widget _buildSpecCard(ServiceData service) {
    ...
    // "Services Offered" merged with "Types of Work" (deduped). Prefer the
    // provider's explicit `expertise` list when present (the design labels this
    // box "Expertise"), else fall back to the combined services set.
          ...
          children: [
            ...
            Stack(
              children: [
                ...
                // Share + Save, stacked top-right (design). Availability is now
                // conveyed by the "Open | hours" chip below, so the standalone
                // live badge is dropped from the card hero.
                Positioned(
                  ...
                  child: Column(
                    children: [
                      ...
                      Obx(() {
                        final saved =
                            controller.isProviderLocallySaved(service.id);
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // Avatar and name open the poster's profile (personal
                  // or business, per account type); the rest of the card
                  // still opens the service detail.
                  Row(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      DiscoverProfileTap(
