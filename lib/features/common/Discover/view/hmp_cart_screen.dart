...
/// Multi-store cart for the home made product flow (Zomato-style). Lists every
/// seller the user has items from, each as its own card with a stepper list
/// and its **own Checkout** button. A bottom **Checkout All** bar appears when
/// there's more than one seller.
class HmpCartScreen extends StatelessWidget {
  const HmpCartScreen({super.key});
  ...
  // ── One item row ───────────────────────────────────────────────────────────
  Widget _cartLine(
      HmpCartController c, EarnProfileModel? store, GetProductData item) {
  ...
  Widget _stepper(
      HmpCartController c, EarnProfileModel? store, GetProductData item) {
    final qty = c.qty(HmpCartController.idOf(item));
    return Container(
      decoration: BoxDecoration(
        borderRadius: BorderRadius.circular(10),
  ...
  // ── Bottom "Checkout All" bar (only with >1 seller) ────────────────────────
  Widget _checkoutAllBar(HmpCartController c) {
            ...
            children: [
              Expanded(
                child: Column(
                  ...
                  children: [
                    CustomText(
                      '${c.totalItems} ${c.totalItems == 1 ? 'item' : 'items'} · ${c.storeCount} sellers',
                      fontSize: SizeConfig.small,
                      color: AppColors.secondaryTextColor,
                      fontWeight: FontWeight.w500,
                    ),
