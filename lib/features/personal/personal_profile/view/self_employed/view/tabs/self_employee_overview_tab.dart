...
/// **Overview tab** of the self-employed dashboard — the identity dossier:
///
///   1. Single hero card unifying the cover banner, the avatar (straddling the
///      photo/sheet boundary) and the identity block (designation eyebrow →
///      name → username · member-since → location/email rows). A hairline
///      divides photo from sheet.
///   2. Stats card with hero-typed numerals and primary-colour accent bars
///      stamped beneath each metric.
...
/// [ViewPersonalDetailsController] / [PersonalCreateProfileController]. The
/// first is resolved with `Get.find`, exactly as the screen did: it is
/// registered permanently at startup, and a `getOrPut` here would quietly
/// construct a second one if that ever stopped being true, hiding the breakage
/// instead of surfacing it.
class SelfEmployeeOverviewTab extends StatelessWidget {
  const SelfEmployeeOverviewTab({super.key});
  ...
  Widget build(BuildContext context) {
    return SelfEmployeeTabScroll(child: _buildOverviewTab(context));
  }
