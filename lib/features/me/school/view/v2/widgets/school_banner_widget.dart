...
/// School cover-banner block — visually mirrors the medical/hospital v2
/// banner widget. Tap → picker → compress → upload via the existing
/// `uploadSchoolLogoOrBannerImage(uploadVia: 'bannerUrl')` endpoint.
class SchoolBannerWidget extends StatefulWidget {
  final SchoolAboutUsController controller;

  const SchoolBannerWidget({super.key, required this.controller});

  @override
  State<SchoolBannerWidget> createState() => _SchoolBannerWidgetState();
}

class _SchoolBannerWidgetState extends State<SchoolBannerWidget> {
  ...
  Future<void> _onEditCover() async {
    ...
    try {
      ...
      await widget.controller.getSchoolByIdController();
    } catch (_) {
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Obx(() {
        final cover = widget.controller.schoolDetailsData?.value.bannerUrl ?? '';
