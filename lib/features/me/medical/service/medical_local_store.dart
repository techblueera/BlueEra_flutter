...
/// One cached medical list, with the moment it was written.
///
...
class MedicalCacheEntry {
  const MedicalCacheEntry({required this.data, required this.savedAt});
...
/// Local (Hive) store for everything the medical admin screens read.
///
/// The medical twin of `GroceryLocalStore` / `FoodLocalStore`, entry for entry
/// — its own box, so one feature's cache can never be read as another's.
///
/// ## What it holds
/// * **Products tab** — the Top Selling list and the my-categories (with
///   inventory) list, one entry each per store.
/// * **Add-medicine flow** — the nested category tree, which is global rather
///   than per store.
///
/// ## Why JSON strings and not raw maps
/// Hive hands back `Map<dynamic, dynamic>` for anything written as a bare map,
/// and `GroceryBusinessProductsModel.fromJson` / `MyMedicalSuperCategoryModel
...
/// `MedicalController.markInventoryChanged()`, which deletes the snapshot and
/// refetches. The one exception is the category tree — no local action can
/// invalidate it, so it carries [catalogTtl].
///
/// ## Lifetime
...
class MedicalLocalStore {
  const MedicalLocalStore._();

  static const String boxName = 'medical_local_cache_box';

  /// Life of the add-medicine category tree before the network is
  /// consulted.
  ...
  /// Catalogue-variant ids the pharmacy already stocks — see
  /// [MedicalController.fetchStockedVariantIdsIfNeeded]. Per store, like the
  ...
  static const String _kCatalog = 'catalog:nestedCategories';
  ...
  static Future<Box?> _safeBox() async {
    try {
      ...
      log('MedicalLocalStore: box unavailable — $e');
      return null;
    }
  }
  ...
  static Future<MedicalCacheEntry?> readTopSelling(
    String storeId, {
  ...
  static Future<MedicalCacheEntry?> readCategories(
    String storeId, {
    required bool otherStore,
  }) =>
  ...
  static Future<MedicalCacheEntry?> readStockedVariantIds(String storeId) =>
  ...
  // ─── Add-medicine category tree (global) ────────────────────────

  static Future<MedicalCacheEntry?> readCatalogCategories() => _read(_kCatalog);
  ...
  static Future<MedicalCacheEntry?> readCatalogChild(String key) =>
  ...
  /// Drops both Products-tab caches for one store, in both scopes. Used when the
  ...
  static Future<void> clearStore(String storeId) async {
    ...
    try {
      ...
      log('MedicalLocalStore.clearStore error: $e');
    }
  }
  ...
  static Future<void> clearAll() async {
    ...
    try {
      ...
      log('MedicalLocalStore.clearAll error: $e');
    }
  }

  // ─── Storage ─────────────────────────────────────────────────────

  static Future<MedicalCacheEntry?> _read(String key) async {
    final box = await _safeBox();
    ...
    try {
      ...
      return MedicalCacheEntry(
        data: payload,
        savedAt: DateTime.fromMillisecondsSinceEpoch(savedAt),
      );
    } catch (e) {
      log('MedicalLocalStore._read($key) error: $e');
      return null;
    }
  }
  ...
  static Future<void> _write(String key, dynamic data) async {
    ...
    try {
      ...
      log('MedicalLocalStore._write($key) error: $e');
    }
  }
  ...
  static Future<void> _prune(Box box) async {
    try {
      ...
      log('MedicalLocalStore._prune error: $e');
    }
  }
