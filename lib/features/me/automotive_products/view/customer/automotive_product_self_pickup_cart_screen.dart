...
/// Place Order CTA.
class AutomotiveProductSelfPickUpCartScreen extends StatefulWidget {
  const AutomotiveProductSelfPickUpCartScreen({super.key});

  @override
  State<AutomotiveProductSelfPickUpCartScreen> createState() =>
      _AutomotiveProductSelfPickUpCartScreenState();
}

class _AutomotiveProductSelfPickUpCartScreenState
    extends State<AutomotiveProductSelfPickUpCartScreen> {
  ...
  Map<String, List<GetProductData>> _groupByBusiness(
      AutomotiveProductSelfPickupController controller) {
  ...
  double _calcTotal(
      List<GetProductData> items, AutomotiveProductSelfPickupController controller) {
    double total = 0;
  ...
  Widget build(BuildContext context) {
    final controller = Get.find<AutomotiveProductSelfPickupController>();
...
class _AutomotiveStoreGroupCard extends StatelessWidget {
  final String businessName;
  final String businessLogo;
  final String businessAddress;
  final List<GetProductData> items;
  final AutomotiveProductSelfPickupController controller;
  ...
  const _AutomotiveStoreGroupCard({
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          if (averageDiscount > 0) _AutomotiveDiscountRibbon(percent: averageDiscount),
        ],
      ),
    );
  }
...
/// the grocery cart.
class _AutomotiveVerticalDashedLine extends StatelessWidget {
  final double height;
  const _AutomotiveVerticalDashedLine({required this.height});
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      width: 1,
      height: height,
      child: CustomPaint(painter: _AutomotiveVerticalDashedLinePainter()),
    );
  }
}

class _AutomotiveVerticalDashedLinePainter extends CustomPainter {
  ...
  @override
  bool shouldRepaint(covariant _AutomotiveVerticalDashedLinePainter old) => false;
}
...
class _AutomotiveProductRow extends StatelessWidget {
    ...
    return AnimatedContainer(
      ...
      child: Row(
        ...
        children: [
          ...
          const SizedBox(width: 10),
          const _AutomotiveVerticalDashedLine(height: 60),
          const SizedBox(width: 10),
          Column(
...
class _AutomotiveQtyStepper extends StatelessWidget {
  final int quantity;
  final VoidCallback onAdd;
  final VoidCallback onRemove;

  const _AutomotiveQtyStepper({
...
class _AutomotiveDiscountRibbon extends StatelessWidget {
  final double percent;
  const _AutomotiveDiscountRibbon({required this.percent});
...
class _AutomotiveBottomBillSection extends StatelessWidget {
  final Map<String, List<GetProductData>> grouped;
  final AutomotiveProductSelfPickupController controller;
  final double Function(List<GetProductData>, AutomotiveProductSelfPickupController)
      calcTotal;
  final int Function(List<GetProductData>, AutomotiveProductSelfPickupController)
      calcItemCount;
  final RxSet<String> selectedBusinessIds;
  final VoidCallback onPlaceOrder;

  const _AutomotiveBottomBillSection({
