...
/// differences: they already diverge on where a tap lands and on which service
/// answers for their inventory, and separate files mean neither has to reason
/// about the others before changing its own list.
///
...
class AutomotiveStoreCard extends StatelessWidget {
  const AutomotiveStoreCard({super.key, required this.store});
  ...
  String get _heroTag => 'automotive_live_${store.id ?? store.userId ?? ''}';
  ...
  /// wasn't.
  /// The automotive store screen hydrates itself from the owner's user id
  /// alone.
  void _openStore() {
