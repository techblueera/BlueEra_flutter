...
enum _AutomotiveAddProductMode { textSearch, snapSearch }

class AutomotiveAddProductTextOrSnapSearchScreen extends StatefulWidget {
  final String id;
  final ProviderType providerType;
  const AutomotiveAddProductTextOrSnapSearchScreen(
      {super.key, required this.id, required this.providerType});

  @override
  State<AutomotiveAddProductTextOrSnapSearchScreen> createState() =>
      _AutomotiveAddProductTextOrSnapSearchScreenState();
}
...
class _AutomotiveAddProductTextOrSnapSearchScreenState
  ...
  void _navigateToCart() {
    // Sync selected variants to AutomotiveProductController for the cart screen
    ...
    for (final v in controller.selectedVariantsList) {
      final inventoryCtrl = Get.find<AutomotiveInventoryController>();
      inventoryCtrl.variantSelection[v.id] = true;
    }

    Get.toNamed(RouteHelper.getAutomotiveAddProductVariantScreenRoute());
  }
  ...
  void _openPreview(AutomotiveSelectedVariant variantData) {
    ...
    final args = AutomotiveProductPreviewArgs(
  ...
  Widget _buildModeTab({
    ...
    required _AutomotiveAddProductMode mode,
  }) {
  ...
  Widget _buildSearchResults() {
    ...
    if (controller.ProductSearchLoading.isTrue || searchPending) {
      return const Padding(
        ...
        child: _AutomotiveVariantGridSkeleton(count: 4),
      );
    }
    ...
    if (controller.searchProductVariantsList.isEmpty) {
      return _AutomotiveSearchEmptyState(
      ...
      children: [
        ...
        if (controller.isLoadingMore)
          const Padding(
            ...
            child: _AutomotiveVariantGridSkeleton(count: 2),
          ),
      ],
    );
  }
      ...
      child: SizedBox(
        ...
        child: ListView.separated(
          ...
          itemBuilder: (_, __) => SizedBox(
            width: SizeConfig.size140,
            child: _AutomotiveQuickAddRailSkeletonCard(),
          ),
        ),
      ),
    );
  ...
  Widget _buildSnapProductCard(AutomotiveSelectedVariant variantData) {
    final product = variantData.product;
    final variant = variantData.variant;
  ...
  Widget _buildVariantGrid(List<AutomotiveSelectedVariant> variants) {
    return Obx(() => MasonryGridView.count(
          ...
          itemBuilder: (_, index) {
            ...
            return AutomotiveProductVariantGridCard(
              variantData: variant,
...
class _AutomotiveSearchTypingHint extends StatefulWidget {
  final int typed;
  final int required;
...
// API call. Mimics the real AutomotiveProductVariantGridCard layout (image ▸
...
class _AutomotiveVariantGridSkeleton extends StatefulWidget {
  final int count;
...
class _AutomotiveSkeletonVariantCard extends StatelessWidget {
  final Animation<double> animation;

  const _AutomotiveSkeletonVariantCard({required this.animation});
              ...
              children: const [
                ...
                Padding(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      _AutomotiveSkBar(width: double.infinity, height: 10),
                      SizedBox(height: 8),
                      _AutomotiveSkBar(width: 80, height: 10),
                      SizedBox(height: 14),
                      _AutomotiveSkBar(width: 64, height: 14),
                    ],
                  ),
                ),
              ],
...
class _AutomotiveSearchEmptyState extends StatelessWidget {
...
class _AutomotiveSearchErrorState extends StatelessWidget {
  final VoidCallback onRetry;

  const _AutomotiveSearchErrorState({required this.onRetry});
...
class _AutomotiveSkBar extends StatelessWidget {
  final double width;
  final double height;

  const _AutomotiveSkBar({required this.width, required this.height});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      height: height,
      decoration: BoxDecoration(
        color: _AutomotiveSkeletonVariantCard._base,
...
class _AutomotiveQuickAddRailSkeletonCard extends StatelessWidget {
  static const Color _base = Color(0xFFEDEFF3);

  const _AutomotiveQuickAddRailSkeletonCard();
