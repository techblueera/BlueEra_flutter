...
class AutomotiveProductPreviewArgs {
  ...
  final String? link;
  final List<AutomotiveDetailPair>? details;
  ...
  final List<AutomotiveProductListing>? listedProducts;

  /// for seeing listed products

  /// Both will be used for available options
  final List<AutomotiveSelectedColor>? selectedColors;

  /// for seeing all products
  ...
  final List<AutomotiveAiVariantData>? variants;

  AutomotiveProductPreviewArgs({
    this.productId,
    this.productImages,
    this.name,
    this.description,
...
class AutomotiveDetailPair {
  final String? title;
  final String? detail;
  const AutomotiveDetailPair(this.title, this.detail);
}

class AutomotiveProductPreviewScreen extends StatefulWidget {
  final String? id;
  final ProviderType? providerType;
  final AutomotiveProductPreviewArgs? productPreviewArgs;

  /// True when reached right after creating a product (AI flow). Drives both
  /// the back-navigation behavior and whether the "Create AutomotiveVariant" CTA shows.
  final bool isFromProductCreation;

  const AutomotiveProductPreviewScreen(
      {super.key,
      required this.id,
...
class _AutomotiveProductPreviewScreenState
  ...
  void loadPreviewArgs(AutomotiveProductPreviewArgs args) {
    ...
    controller.featureControllers
      ..clear()
      ..addAll(
          (args.features ?? []).map((f) => TextEditingController(text: f)));
    ...
    controller.userGuideLineControllers
      ..clear()
      ..addAll(
          (args.userGuide ?? []).map((g) => TextEditingController(text: g)));
...
// """);
  ...
  @override
  void dispose() {
    deleteIfRegistered<AutomotiveProductController>();
    super.dispose();
  }
              ...
              children: [
                              ...
                              CarouselSlider.builder(
                                ...
                                itemBuilder: (context, index, realIdx) {
                                  return !isNetworkImage(
                                          controller.allProductImages[index])
                                      ? Image.file(
                                          File(controller
                                  ...
                                  ? Positioned(
                                      ...
                                      child: Row(
                                        ...
                                        children: List.generate(
                                            controller.allProductImages.length,
                                            (index) {
                ...
                CustomFormCard(
                  ...
                  child: Column(
                    ...
                    children: [
                      CustomText(
                        'Here Is Your AutomotiveProduct details',
                        fontSize: SizeConfig.large,
                        fontWeight: FontWeight.w600,
                        color: AppColors.mainTextColor,
                      ),
            ...
            children: controller.dynamicAttributes.entries.map((entry) {
              ...
              return Column(
                ...
                children: [
                  ...
                  SizedBox(
                      height: SizeConfig
                          .size8), // space between different attributes
                ],
              );
            }).toList(),
          )
  ...
  // specification chips.
  Widget _responseVariantCard(AutomotiveAiVariantData v) {
  ...
  Widget ProductCard(
    AutomotiveProductListing product, {
      ...
      return GestureDetector(
        onTap: () {
          ...
          if (isSelected) {
            ...
            controller.selectedProductOrVariantPrice.value =
                product.price ?? '00,000';
            controller.selectedProductOrVariantMrp.value =
                product.mrp ?? '00,000';
            controller.selectedProductOrVariantDiscount.value =
                product.discount ?? '0';
          }
        },
        child: Container(
            ...
            child: Column(
              ...
              children: [
                ...
                if (selectedVariants != null && selectedVariants.isNotEmpty)
                  ...
                  const CustomText(AppStrings.noVariantsSelected,
  ...
  Widget _buildVariantRow(String key, dynamic value) {
    ...
    if (key == 'color' && value is Map<String, dynamic>) {
      final color = AutomotiveSelectedColor.fromJson(value);
