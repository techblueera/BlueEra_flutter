...
// `show` keeps `AutomotiveVariant` / `AutomotiveProductFeature` from this module out of the
...
class AutomotiveInventoryController extends GetxController {
  ...
  RxList<AutomotiveProductModel> products = <AutomotiveProductModel>[].obs;
  RxList<AutomotiveProductModel> filteredProducts = <AutomotiveProductModel>[].obs;
  RxList<AutomotiveCategoryInventoryModel> categories = <AutomotiveCategoryInventoryModel>[].obs;
  RxList<AutomotiveCategoryInventoryModel> filteredCategories = <AutomotiveCategoryInventoryModel>[].obs;
  ...
  RxList<AutomotiveSelectedVariant> searchProductVariantsList = <AutomotiveSelectedVariant>[].obs;

  final variantSelection = <String, bool>{}.obs;
  final variantSellingPrice = <String, String>{}.obs;

  /// User-overridden MRP per variant id. The base [AutomotiveVariant.mrp] is immutable
  ...
  RxList<AutomotiveSelectedVariant> selectedVariantsList = <AutomotiveSelectedVariant>[].obs;
  ...
  void toggleVariantWithData(AutomotiveSelectedVariant selected) {
  ...
  double effectiveMrp(AutomotiveSelectedVariant row) {
  ...
  RxList<AutomotiveSelectedVariant> suggestedProductList = <AutomotiveSelectedVariant>[].obs;

  // ── AutomotiveProduct Nested Category With Inventory ──────────────────────────────────
  ...
  /// 2. [AutomotiveLocalStore] — the saved snapshot. **If there is one, that is
  ...
  /// price edit, delete) runs [markInventoryChanged], which refetches and
  /// rewrites it — so the only way to be looking at stale stock is for it to
  ...
  Future<void> fetchAllProductDataIfNeeded({String? visitUserId}) async {
    ...
    if (await _hydrateProductDataFromCache(visitUserId)) {
  ...
  Future<bool> _hydrateProductDataFromCache(String? visitUserId) async {
    ...
    if (visitUserId != null || userId.isEmpty) return false;

    List<AutomotiveProductCategoryWithInventoryModel>? categories;
    List<GetProductData>? topSelling;
    try {
      final entry =
          await AutomotiveLocalStore.readCategories(userId, otherStore: false);
      if (entry != null && !entry.isEmpty) {
        categories = _parseProductCategories(entry.items);
      }
    } catch (e) {
      log('automotive: category cache hydrate failed — $e');
    }
    try {
      final entry =
          await AutomotiveLocalStore.readTopSelling(userId, otherStore: false);
      if (entry != null && !entry.isEmpty) {
        topSelling = GetProductModel.fromJson({'data': entry.items}).data;
      }
    } catch (e) {
      log('automotive: top-selling cache hydrate failed — $e');
    }
  ...
  /// price / delete) that the merchant just made.
  ///
  ...
  void markInventoryChanged() {
    ...
    unawaited(AutomotiveLocalStore.clearStore(userId).then(
  ...
  List<AutomotiveProductCategoryWithInventoryModel> _parseProductCategories(
          List<dynamic> rawList) =>
      rawList
          .whereType<Map>()
          .map((e) => AutomotiveProductCategoryWithInventoryModel.fromJson(
  ...
  Future<void> fetchProductCategoryWithInventory({
    ...
    try {
      ...
      if(visitUserId!=null){
        response = await AutomotiveProductRepo().fetchPublicProductCategoryWithInventoryRepo(visitUserId: visitUserId);
      }else{
        response = await AutomotiveProductRepo().fetchProductCategoryWithInventoryRepo();
      }
      ...
      if (response.isSuccess) {
        ...
        if (visitUserId == null && userId.isNotEmpty) {
          unawaited(AutomotiveLocalStore.writeCategories(
            userId,
            otherStore: false,
            items: rawList,
          ));
  ...
  // ── AutomotiveProduct By Category (Paginated) ────────────────────────────────
  final selectedProductCategoryData =
      Rxn<AutomotiveProductCategoryWithInventoryModel>();
  ...
  Future<void> fetchProductsByCategory({
    ...
    try {
      ...
      final response = await AutomotiveProductRepo().fetchProductsRepo(queryParams: queryParams);
  ...
  RxList<AutomotiveSelectedVariant> snapSearchProductList = <AutomotiveSelectedVariant>[].obs;
  ...
  Future<void> fetchProductSnapSearchApi() async {
    ...
    try {
      ...
      final responseModel =
          await AutomotiveProductRepo().fetchProductSnapSearchRepo(params: params);

      if (responseModel.isSuccess) {
        productSnapSearchResponse.value = ApiResponse.complete(responseModel);

        final response = AutomotiveProductSnapSearchResponseModel.fromJson(
          responseModel.response?.data,
        );
        productSnapSearchData.value = response.data;

        final foundItems = response.data?.foundProducts ?? [];
        final rows = <AutomotiveSelectedVariant>[];
        for (final item in foundItems) {
          final product = item.product;
          if (product == null) continue;
          for (final v in product.variants) {
            rows.add(AutomotiveSelectedVariant(product: product, variant: v));
          }
        }
      ...
      productSnapSearchResponse.value = ApiResponse.error('error');
      log("AutomotiveProduct Snap Search Error: $s");
    }
  }
        ...
        if (isLoadMore) {
            ...
            if (visitUserId == null &&
              ...
              final rawItems = (raw is Map && raw['data'] is List)
                  ? raw['data'] as List
                  : const [];
              unawaited(AutomotiveLocalStore.writeTopSelling(
                userId,
                otherStore: false,
                items: rawItems,
              ));
  ...
  Future<void> fetchListOfSearchProductApi(String keyword, {bool isLoadMore = false}) async {
    ...
    try {
      ...
      final responseModel = await AutomotiveProductRepo().fetchSearchProductViaCategoryRepo(queryParams: params);

      if (responseModel.isSuccess) {
        searchProductResponse.value = ApiResponse.complete(responseModel);

        final parsed = AutomotiveProductCatalogResponse.fromJson(
          responseModel.response?.data,
        );
        ...
        final Map<String, AutomotiveSelectedVariant> uniqueById = {
  ...
  List<String> validateSelectedVariants(List<AutomotiveSelectedVariant> allVariants) {
  ...
  void fillMissingSellingPricesWithDefaults(
      List<AutomotiveSelectedVariant> allVariants, List<String> missingPriceIds) {
    for (final variantId in missingPriceIds) {
      final row = allVariants.firstWhere(
        (v) => v.id == variantId,
        orElse: () => throw Exception("AutomotiveVariant not found: $variantId"),
      );
  ...
  Future<void> fetchListOfSuggestedProductApi({bool isLoadMore = false}) async {
    ...
    try {
      ...
      final responseModel = await AutomotiveProductRepo().fetchSuggestedProductRepo(queryParams: params);
  ...
  Future<void> cloneProductVariantApi(
      {
        ...
        required List<AutomotiveSelectedVariant> variants,
      }
      ) async {
    ...
    try {
      ...
      if (responseModel.isSuccess) {
        ...
        // Wipe the cart after a successful publish. The AutomotiveInventoryController is
  ...
  /// overrides. Also clears the sibling [AutomotiveProductController.selectedProducts]
  ...
  void _clearPublishCart() {
    ...
    if (Get.isRegistered<AutomotiveProductController>()) {
      Get.find<AutomotiveProductController>().selectedProducts.clear();
    }
  }
  ...
  void openAddVariantDialog({
    ...
    showDialog(
      ...
      builder: (_) {
        return AutomotiveProductVariantDialog(
  ...
  Future<void> createNewProductVariantApi({
    ...
    try {
      ...
      final response = await AutomotiveProductRepo().createNewProductVariantRepo(
        productId: productId,
        params: data,
      );
      ...
      markInventoryChanged();
      commonSnackBar(message: 'AutomotiveVariant added successfully');
      Get.back();
    } catch (e, s) {
  ...
  void navigateToProductSection() {
    ...
    List<Map<String, dynamic>> _buildInventoryPayload(
        List<AutomotiveSelectedVariant> allVariants, ProviderType providerType) {
  ...
  String _resolveVariantQuantity(AutomotiveVariant variant) {
  ...
  Future<bool> updateProductVariantPrice({
    ...
    try {
      final res = await AutomotiveProductRepo().updateInventoryVariantRepo(
