...
///
/// Mirrors the API JSON 1:1 — one [AutomotiveProduct] per `data[]` entry with a
/// nested `variants[]` list. Consumers that need "one row per variant"
/// (selection sets, cart rows) wrap each pair in [AutomotiveSelectedVariant].
///
...
/// }
/// ```
class AutomotiveProductCatalogResponse {
  final List<AutomotiveProduct> data;
  final AutomotivePagination? pagination;

  AutomotiveProductCatalogResponse({
    required this.data,
    this.pagination,
  });

  factory AutomotiveProductCatalogResponse.fromJson(
      Map<String, dynamic> json) {
    final rawData = (json['data'] as List<dynamic>?) ?? const [];
    final products = <AutomotiveProduct>[];
    for (final raw in rawData) {
      if (raw is! Map) continue;
      products.add(AutomotiveProduct.fromJson(Map<String, dynamic>.from(raw)));
    }
    final paginationRaw = json['pagination'];
    return AutomotiveProductCatalogResponse(
      data: products,
      pagination: paginationRaw is Map
          ? AutomotivePagination.fromJson(Map<String, dynamic>.from(paginationRaw))
          : null,
    );
  }
}
...
class AutomotiveProduct {
  ...
  final List<AutomotiveProductDetail> additionalDetails;
  final List<AutomotiveProductFeature> features;
  ...
  factory AutomotiveProduct.fromJson(Map<String, dynamic> json) {
    ...
    return AutomotiveProduct(
      ...
      additionalDetails: additionalRaw
          .whereType<Map>()
          .map((e) => AutomotiveProductDetail.fromJson(Map<String, dynamic>.from(e)))
          .toList(),
      features: featuresRaw
          .whereType<Map>()
          .map((e) => AutomotiveProductFeature.fromJson(Map<String, dynamic>.from(e)))
          .toList(),
      variants: variantsRaw
          .whereType<Map>()
          .map((e) => AutomotiveVariant.fromJson(Map<String, dynamic>.from(e)))
          .toList(),
    );
  }
}
...
class AutomotiveProductDetail {
  ...
  AutomotiveProductDetail({required this.title, required this.details});
...
class AutomotiveProductFeature {
  ...
  AutomotiveProductFeature({required this.title});

  factory AutomotiveProductFeature.fromJson(Map<String, dynamic> json) {
    return AutomotiveProductFeature(title: (json['title'] ?? '').toString());
  }
}

class AutomotivePagination {
  final int total;
  final int page;
  final int limit;
  final int totalPages;

  AutomotivePagination({
    required this.total,
    required this.page,
    required this.limit,
  ...
  factory AutomotivePagination.fromJson(Map<String, dynamic> json) {
    int asInt(dynamic v) =>
        v is int ? v : int.tryParse('${v ?? 0}') ?? 0;
    return AutomotivePagination(
...
/// rows, and any UI that paints a card per [AutomotiveVariant] while still
/// needing the parent [AutomotiveProduct] for name/media/preview metadata.
class AutomotiveSelectedVariant {
  final AutomotiveProduct product;
  final AutomotiveVariant variant;

  const AutomotiveSelectedVariant({required this.product, required this.variant});
...
List<AutomotiveSelectedVariant> flattenProducts(List<AutomotiveProduct> products) {
  final out = <AutomotiveSelectedVariant>[];
  for (final p in products) {
    if (p.variants.isEmpty) continue;
    for (final v in p.variants) {
      out.add(AutomotiveSelectedVariant(product: p, variant: v));
    }
  }
  return out;
}
