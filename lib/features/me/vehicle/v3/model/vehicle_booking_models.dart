/// Domain models for the vehicle **booking** — conceptually an *enquiry* —
/// flow (`vehicle-service/bookings/...`).
///
/// One request = one listing: no cart, no quantity, no payment. Every model
/// exposes `fromJson`; request bodies are assembled from primitive args.
///
/// These survived the v3 rebuild unchanged because they describe the **chat
/// message payload** (an enquiry card in a business chat), not a response
/// from the removed `/vehicles/*` API — and the intents and statuses the
/// rebuilt `/bookings` contract uses are the same set. See
/// docs/backend/FRONTEND_INTEGRATION_GUIDE.md §7.
library;
