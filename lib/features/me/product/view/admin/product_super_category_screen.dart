...
/// Add-product landing: an "Add Products" snap card followed by "Quick Upload"
/// rails — one horizontal rail per root category (title + icon + "More"), each
/// showing products from `products/by-root-category`. Tapping a card opens the
/// variant picker sheet; selected variants accumulate in the controller and the
/// floating cart routes to the Add Product Variant screen.
...
class _ProductSuperCategoryScreenState
  ...
  Widget _buildScrollBody() {
    return Obx(() {
      final resp = controller.productRootCategoryResponse.value;
  ...
  /// states.
  Widget _railsSliver(
      ApiResponse resp, List<ProductRootCategorySection> sections) {
    if (sections.isEmpty) {
  ...
  /// over a horizontal list of [ProductSelectionProductCard]s.
  Widget _rootCategorySection(ProductRootCategorySection section) {
    return Container(
              ...
              children: [
                for (int i = 0; i < section.products.length; i++) ...[
                  ...
                  SizedBox(
                    ...
                    child: ProductSelectionProductCard(
  ...
  Widget _railHeader(ProductRootCategorySection section) {
    return Padding(
  ...
  void _openCategory(ProductRootCategorySection section) {
    Get.toNamed(
