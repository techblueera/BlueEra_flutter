...
class ProductCacheEntry {
  const ProductCacheEntry({required this.data, required this.savedAt});
...
/// Local (Hive) store for everything the product admin screens read.
///
/// The product twin of `GroceryLocalStore` / `FoodLocalStore`, entry for entry
...
/// `InventoryController.markInventoryChanged()`, which deletes the snapshot and
/// refetches. The one exception is the super-category list — no local action
...
class ProductLocalStore {
  const ProductLocalStore._();

  static const String boxName = 'product_local_cache_box';
  ...
  static Future<Box?> _safeBox() async {
    try {
      ...
      log('ProductLocalStore: box unavailable — $e');
      return null;
    }
  }
  ...
  static Future<ProductCacheEntry?> readTopSelling(
    String storeId, {
  ...
  static Future<ProductCacheEntry?> readCategories(
    String storeId, {
    required bool otherStore,
  }) =>
  ...
  static Future<ProductCacheEntry?> readCatalogChild(String key) =>
  ...
  static Future<void> clearAll() async {
    ...
    try {
      ...
      log('ProductLocalStore.clearAll error: $e');
    }
  }

  // ─── Storage ─────────────────────────────────────────────────────

  static Future<ProductCacheEntry?> _read(String key) async {
    final box = await _safeBox();
    ...
    try {
      ...
      log('ProductLocalStore._read($key) error: $e');
      return null;
    }
  }
  ...
  static Future<void> _write(String key, dynamic data) async {
    ...
    try {
      ...
      log('ProductLocalStore._write($key) error: $e');
    }
  }
  ...
  static Future<void> _prune(Box box) async {
    try {
      ...
      log('ProductLocalStore._prune error: $e');
    }
  }
