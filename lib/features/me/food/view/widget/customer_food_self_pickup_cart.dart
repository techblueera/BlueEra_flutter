...
/// Zomato-style floating cart for food self-pickup.
///
/// Collapsed it's a compact pill that previews the cart's food photos.
/// Tapping it makes a "Your Carts (N)" panel **rise up from the bottom**;
/// closing slides it back down. Each restaurant shows small circular product
...
class CustomerFoodSelfPickupCart extends StatefulWidget {
  ...
  const CustomerFoodSelfPickupCart({
    super.key,
...
class _CustomerFoodSelfPickupCartState
  ...
  void _openCartScreen() {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => const FoodSelfPickUpCartScreen()),
    );
  }
  ...
  Widget _buildCollapsed(FoodSelfPickupController c, Key key) {
    final stores = c.storeCount;
    final items = c.totalItemsCount;
    final total = c.totalSellingPrice;
    final imgs =
        c.selectedFoodVariants.take(3).map((v) => c.imageOf(v)).toList();
  ...
  Widget _buildPanel(FoodSelfPickupController c, Key key) {
  ...
  /// product photos.
  Widget _storeChip(FoodSelfPickupController c, String bid) {
  ...
  Widget _circle(String? url, double size,
    ...
    final fallback = Container(
      ...
      alignment: Alignment.center,
      child: Icon(
        storeFallback ? Icons.storefront_rounded : Icons.fastfood_rounded,
        size: size * 0.42,
