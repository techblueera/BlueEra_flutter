...
class _LabAvailabilityScreenState extends State<LabAvailabilityScreen> {
  ...
  // to size itself — so every field on this screen built all 48 options before
  // the screen could show anything, each an Icon plus a [CustomText] (which
  // runs a `.tr` lookup per build). That is what made the hours editor take
  // seconds to appear. Now the closed field is a label, and the 48 rows are
  // built lazily, for one field, only when tapped. Same fix as
  // `ShopAvailabilityScreen`, which this screen mirrors.
  Widget _timeField({
    ...
    return Column(
      ...
      children: [
        ...
        InkWell(
          ...
          // still be on top when the pop lands.
          onTap: _isSaving
              ? null
              : () => _pickTime(
  ...
  Future<void> _onSave() async {
    ...
    // `Get.back` pops whatever route is topmost, not this screen's, and the
    // route casts the result to its own type. Clear anything above us first
    // with a null result — valid for every route here — so the `true` can only
    // reach the route it is addressed to.
