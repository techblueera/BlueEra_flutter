...
/// Zomato-style floating cart for grocery self-pickup.
///
...
class CustomerGrocerySelfPickupCart extends StatefulWidget {
  final GrocerySelfPickupConsumerController controller;

  const CustomerGrocerySelfPickupCart({
    super.key,
    required this.controller,
  });

  @override
  State<CustomerGrocerySelfPickupCart> createState() =>
      _CustomerGrocerySelfPickupCartState();
}

class _CustomerGrocerySelfPickupCartState
    extends State<CustomerGrocerySelfPickupCart> {
  ...
  void _openCartScreen() {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => const GrocerySelfPickUpCartScreen()),
    );
  }
  ...
  Widget _buildCollapsed(GrocerySelfPickupConsumerController c, Key key) {
    final stores = c.storeCount;
    final items = c.totalItemsCount;
    final total = c.totalSellingPrice;
    final imgs =
        c.selectedGroceriesVariants.take(3).map((v) => c.imageOf(v)).toList();
  ...
  Widget _buildPanel(GrocerySelfPickupConsumerController c, Key key) {
  ...
  /// product photos.
  Widget _storeChip(GrocerySelfPickupConsumerController c, String bid) {
    ...
    final imgs = c.variantsOf(bid).map((v) => c.imageOf(v)).toList();
