import 'package:get/get.dart';
import 'package:BlueEra/features/me/job_seekar/controller/job_seeker_portfolio_professionals_controller.dart';

/// Registers [JobSeekerPortfolioProfessionalsController].
///
/// `fenix: true`: it is also read outside the screen that owns it, so the
/// builder must survive a route pop and rebuild on the next Get.find.
///
/// NOTE: nothing calls Get.isRegistered<JobSeekerPortfolioProfessionalsController>() — that was checked before
/// binding it. A binding (fenix especially) keeps the key registered, which would
/// permanently flip any such guard.
class JobSeekerPortfolioProfessionalsBinding extends Bindings {
  @override
  void dependencies() {
    Get.lazyPut<JobSeekerPortfolioProfessionalsController>(
        () => JobSeekerPortfolioProfessionalsController(),
        fenix: true);
  }
}
