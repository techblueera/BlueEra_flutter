...
import 'package:BlueEra/features/me/medical_new/model/medical_available_item_model.dart';
import 'package:BlueEra/features/me/medical_new/model/medical_order_model.dart';
...
class MedicalOrdersController extends GetxController{
  ...
  RxList<MedicalOrdersModel> groceryPendingOrders = <MedicalOrdersModel>[].obs;

  /// Bill Details
  final GlobalKey<FormState> formKey = GlobalKey<FormState>();
  final groceryAvailableItemModel = Rxn<MedicalAvailableItemModel>();
 ...
 // 3. Helper to check status (Use this in your UI)
  ...
  Future<void> fetchGroceryOrders({required String groceryOrderStatus}) async {
    try {
      ...
      final response = await MedicalRepo().fetchGroceryOrdersRepo(
        queryParm: {
  ...
  Future<void> submitCustomerOrderForItemAvailability({required String groceryOrderId}) async {
    try {
      ...
      final response = await MedicalRepo().groceryOrderItemAvailabilityRepo(
        params: {
  ...
  Future<void> getCustomerGroceryOrderAvailableItem({required String groceryOrderId}) async {
    try {
      ...
      final response = await MedicalRepo().groceryOrderAvailableItemRepo(
        queryParams: {
          ApiKeys.businessId : businessId,
          ApiKeys.groceryOrderId : groceryOrderId,
        },
      ...
      if (response.isSuccess) {
        ...
        groceryAvailableItemModel.value = MedicalAvailableItemModel.fromJson(response.response?.data);
      } else {
  ...
  Future<void> submitOrderToRider({
    ...
    if(formKey.currentState!.validate()){
      try {
        ...
        final response = await MedicalRepo().submitGroceryOrderToRiderRepo(
          params: {
            ApiKeys.businessId : businessId,
