...
class _ProfessionalsMainScreenState extends State<ProfessionalsMainScreen> {
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      ...
      final isEarnSelected =
          _selfWorkCtrl.selectedProfileIndex.value == 1 && hasEarnProfile;
      if (isEarnSelected) {
        return const EarnServiceDashboardView(fromBottomNavBar: false);
      }
    ...
    return Obx(() {
      ...
      if (hasEarnProfile) {
        ...
        return EarnServiceProfileSelector(
          ...
          profileNames: ['Skill Work', _earnCtrl.earnProfileLabel(earnType)],
  ...
  // Empty-state CTA shown when the user hasn't yet created a
  // professional profile. Tapping the button posts a minimal record
  // built from whatever they already have on their personal profile
  // (name / email / designation / address etc.). Matches the
  // self-employed dashboard's empty-state design language.
