...
class _ProfessionalsMainScreenState extends State<ProfessionalsMainScreen>
  ...
  bool get _hasWebsite {
    final url =
        _ctrl.getProfessionalServiceRes?.value.data?.contact?.website ?? '';
                ...
                if (hasEarnProfile)
                  Flexible(
                    child: EarnServiceProfileSelector(
                      ...
                      profileNames: [
                        'Skill Work',
                        _earnServiceCtrl.earnProfileLabel(earnType),
                      ],
                      selectedIndex: _selfWorkCtrl.selectedProfileIndex.value,
                      onProfileSelected: (index) =>
                          _selfWorkCtrl.switchProfile(index),
                      onCoverOverlay: true,
                    ),
                  )
                else
  ...
  /// header. Tapping pushes [UpdateProfileScreen] so the user can edit
  /// their personal profile (name, username, designation, etc.) without
  /// leaving the current dashboard.
